...
namespace SoftKata.ExtendedEditorGUI {
    public static partial class ExtendedEditorGUI {
        ...
        public class ListView<TData, TDrawer> : IDrawableElement where TDrawer : IDrawableElement, new() {
            ...
            private int _debugScrollToIndex;
            private void DebugDataGUI() {
                // FSM
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect0)) {
                    EditorGUI.LabelField(rect0, $"Rendering state: {_state}");
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect01)) {
                    EditorGUI.LabelField(rect01, $"Hot control id: {GUIUtility.hotControl}");
                }
                // Animator
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect1_5)) {
                    EditorGUI.LabelField(rect1_5, $"Is animating: {_animator.isAnimating}");
                }
                // Rendering
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect1)) {
                    EditorGUI.LabelField(rect1, $"First visible index: {_firstVisibleIndex}");
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect2)) {
                    EditorGUI.LabelField(rect2, $"Total visible elements: {_visibleElementsCount}");
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect3)) {
                    EditorGUI.LabelField(rect3, $"Total drawers: {_drawers.Count}");
                }
                // Selection
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect4)) {
                    EditorGUI.LabelField(rect4, $"Active index: {_activeElementIndex}");
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect4_1)) {
                    EditorGUI.LabelField(rect4_1, $"Selected items count: {_selectedIndices.Count}");
                }
                // Drag and drop
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect5)) {
                    EditorGUI.LabelField(rect5, $"Drag data validated: {_isDragDataValidated}");
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect6)) {
                    EditorGUI.LabelField(rect6, $"Drag type: {_dragOperationType}");
                }
                // Reordering
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect8)) {
                    EditorGUI.LabelField(rect8, $"Held element y pos: {_activeElementPosY}");
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect9)) {
                    var selectedDrawerIndex = GetDrawerIndexFromDataIndex(_activeElementIndex);
                    var indexString = selectedDrawerIndex != -1 ? selectedDrawerIndex.ToString() : "Invalid";
                    EditorGUI.LabelField(rect9, $"Held element drawer index: {indexString}");
                }
                // Scroll to
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
                if(LayoutEngine.GetRect(18f, -1, out var rect10)) {
                    _debugScrollToIndex = EditorGUI.IntField(rect10, $"Scroll to index:", _debugScrollToIndex);
                }
                if(LayoutEngine.GetRect(18f, -1, out var rect11)) {
                    if(GUI.Button(rect11, "Execute")) {
                        ScrollToAnim(_debugScrollToIndex);
                    }
                }
                EditorGUI.DrawRect(LayoutEngine.GetRect(1), Color.black);
            }
