...
namespace SoftKata.ExtendedEditorGUI {
    public class ExtendedReorderableList : ReorderableList {
        private float _layoutRemovalOffsetY = 7f; 
        ...
        protected Predicate<Object> DragAndDropValidator;
                ...
                case EventType.MouseDown:
                    if (rect.Contains(CurrentEvent.mousePosition)) {
                        if (CurrentEvent.button == 0) {
                            int clickedIndex = GetRowIndexByPosition(rect, CurrentEvent.mousePosition);
